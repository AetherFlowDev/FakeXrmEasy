...
namespace AetherFlow.FakeXrmEasy.Plugins.Tests.FakeContextTests.ReviseQuoteRequestTests
{
    public class ReviseQuoteRequestTests
    {
        ...
        public void Should_Create_New_Quote_With_Lines_When_Revisioning()
        {
            ...
            Assert.AreEqual("Adventure Quote", quote.GetAttributeValue<string>("name"));
            ...
            Assert.AreEqual(1, quoteLines.Count);
            Assert.AreEqual(new Money(1000m), quoteLines.Single().GetAttributeValue<Money>("extendedamount"));
        }
    }
}