...
namespace AetherFlow.FakeXrmEasy.Plugins.Tests.Issues
{
    public class Issue278
    {
        ...
        public void Reproduce_issue_278()
        {
            ...
            Assert.AreEqual(label, statusAttributeMetadata.OptionSet.Options.First().Label.LocalizedLabels[0].Label);
            //Assert.AreEqual(label, statusAttributeMetadata.OptionSet.Options.First().Label.UserLocalizedLabel.Label); This one is null when using the above Label constructor
        }
    }
}
